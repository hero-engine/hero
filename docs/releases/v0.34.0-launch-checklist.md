...
| Check | State | Required action |
|---|---|---|
| Repository visibility change is approved | PASS | The owner approved the bounded history sanitation and archive-and-replace launch; the replacement repository is public and the original remains private. |
| Anonymous clone and source links work | PASS | A credential-free clone, GitHub API check, source route, and public-history scan passed. |
| Docs and landing deploy the approved revision | PASS | Landing and docs production checks match the immutable `v0.34.0` source revision. |
| Public domains and destinations resolve | PASS | DNS/TLS, apex/`www`, docs, issue, vulnerability, support, security, installer, and release routes return successfully. |
| `v0.34.0` tag and release publication are approved | PASS | The immutable tag and published provenance identify the sanitized release revision; five supported archives and trust artifacts are public. |
| Homebrew and Scoop resolve the new checksums | PASS | Both channels identify `v0.34.0`; their target sets and hashes match the public production archives. |
