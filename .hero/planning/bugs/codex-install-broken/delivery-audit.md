...
- [✓] `.codex/config.toml` — `hero:managed` marker added; `command` field changed from `hero` (PATH lookup) to `/Users/bwheeler/go/bin/hero` (absolute path). Note: absolute path is machine-local — see audit notes.
