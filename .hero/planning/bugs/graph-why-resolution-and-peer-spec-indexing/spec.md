...
## Environment Details
- Local dev workspace `/Users/bwheeler/projects/hero-engine/repository/hero`, git remote `git@…:hero-engine/hero.git` → `gitutil.RepoKey` yields `hero-engine/hero`; `filepath.Base(projectRoot)` yields `hero`.
