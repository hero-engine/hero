...
- **Unrelated build break (since resolved).** During delivery, uncommitted `connect_team.go` WIP (team-connect) declared a duplicate `openBrowser`, breaking the `internal/cli` build independently of this work; verification was first run with that one file temporarily stashed. The duplicate has since been removed from the working tree, and the full delivery re-verifies cleanly with no workaround (`go build ./...`, `go vet`, `go test ./internal/cli ./internal/graph` all pass). Nothing in this delivery touched that file.
