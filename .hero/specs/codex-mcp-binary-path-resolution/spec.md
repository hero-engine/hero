...
1. **Wrong-hero.** `LookPath` returns whichever `hero` is first on the installer's PATH,
   ...
   `/Users/bwheeler/go/bin/hero` (`v0.25.1-4-gfa3b339-dirty`) and `/opt/homebrew/bin/hero`
...
```
$ which -a hero
/Users/bwheeler/go/bin/hero        → v0.25.1-4-gfa3b339-dirty
...
content hardcodes `/Users/bwheeler/go/bin/hero`. For any teammate, CI runner, or container,
...
`.mcp.json` (claude) carries the identical `/Users/bwheeler/go/bin/hero` and the identical
...
| File | Lines | Relevance |
|------|-------|-----------|
| `.mcp.json` | 1–9 | Claude config — identical hardcoded `/Users/bwheeler/go/bin/hero`, identical fragility. |
