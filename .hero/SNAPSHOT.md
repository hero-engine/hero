...
|---|---|---|---|---|
| core | maturing | cmd/, internal/ | 3d ago | hero-runner |
| docs | maturing | web/docs/ | 35d ago | — |
| domains/chat | maturing | domains/chat/ | 30d ago | — |
| domains/engineering | maturing | domains/engineering/ | 24d ago | — |
| domains/pm | maturing | domains/pm/ | 30d ago | — |
...
- **Stale-in-flight (4):** `hero-landing-page` (90d), `retrieval-contradiction-detection` (38d), `agent-outposts` (36d), `team-connect` (36d).
- **Aged open bugs (16):** `install-target-emits-both-claude-and-agents-md` (open 94d), `next-project-file-conflict-not-regenerated` (open 75d), `desktop-sidebar-mcp-not-running` (open 74d), `hihcp-agents-md-harness-agnostic` (open 69d), `hihcp-mcp-auto-reconnect` (open 69d), `hihcp-mcp-first-turn-readiness` (open 69d), `hihcp-permission-bridge-validation` (open 69d), `hihcp-agent-loop-error-recovery` (open 69d), `hihcp-rgignore` (open 69d), `jira-connection-onboarding-misleads-agents` (open 34d), `resume-emits-dead-recall-command` (open 34d), `tracker-backed-diagnosis-publication-contract-broken` (open 29d), `tracker-semantic-priority-field-mapping` (open 28d), `jira-import-classification-obscures-work-items` (open 28d), `ledger-signoff-substring-match-fails-open` (open 23d), `graph-unpartitioned-writers-duplicate-nodes` (open 23d).
- **Unassigned specs (245) — no `surface:` declared.** Run `hero snapshot assign` to bucket them.

## Snapshot health

- Surfaces detected: 9 (inferred: 9 · overrides applied: 0)
- Specs covered: 215/460 (46%)
- Projection generation: 1ms · Source nodes: 628
