...
You do not delegate directly. Inputs flow to you via `pm-delivery-lead`, which can pull metrics context, capacity context, or evidence syntheses when needed — from `discovery-researcher`, or pm-delivery-lead loading `metrics-design` / `sprint-planning` (P1 specialists ship v1.5). You receive the inputs and compute the framework.
