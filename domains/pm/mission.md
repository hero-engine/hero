...
| Layer | What's in it |
|---|---|
| **Spec types** | **Shared Core** registered spec types: `feature`, `epic`, `initiative`, `prd`, `intake` — used across domains under the unified type model. **PM-owned extension type:** `roadmap-item`. The PM-common type is `feature` (rendered as "Story" / "Scope" / "Card" depending on active vocabulary); engineering picks up the same feature via the owner-flip workflow. |
