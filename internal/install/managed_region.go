...
// managed_region.go — versioned managed-region primitive used to inject
// Hero-managed content into otherwise user-owned files (AGENTS.md primarily,
// later AGENTS.md siblings like CLAUDE.md shims, and per-harness config
// files like opencode.json/.aider.conf.yml).
//
// Region format:
//
...
// everything outside. Re-running install regenerates the region in place,
// preserving outside content byte-for-byte.
//
// Detection is stateless. The functions here never read remembered state —
// always re-derive from file content. This is what makes user-authored
// content inviolate across upgrades.

const (
	managedStartPrefix = "<!-- hero:managed-start"
	managedEndMarker   = "<!-- hero:managed-end -->"

	// Legacy single-line marker from pre-P1 installs. We recognize it for
	// migration so existing CLAUDE.md / AGENTS.md content can be converted
	// to the versioned form automatically.
	legacyMarker = "<!-- hero:managed -->"
)

// versionedStartRe matches `<!-- hero:managed-start v=<version> -->` and
// captures the version string. Tolerates whitespace inside the comment.
var versionedStartRe = regexp.MustCompile(`<!--\s*hero:managed-start\s+v=([^\s>]+)\s*-->`)

// ManagedRegion describes the position and content of a Hero-managed region
// inside a file.
type ManagedRegion struct {
	...
	// Version is the version stamp from the start marker (empty for legacy
	// regions). Used to detect "managed region is at an older Hero version,
	// needs regeneration."
	Version string

	// StartIdx is the byte offset of the first character of the start marker.
	StartIdx int
	...
	// trimmed of leading/trailing newlines). This is what Hero compares
	// against newly-generated content to detect user edits inside the
	// region.
	Body string
}
...
func FindManagedRegion(content string) ManagedRegion {
	...
	if startIdx < 0 {
		return ManagedRegion{Present: false}
	}
	afterStart := startIdx + len(legacyMarker)
	rel := strings.Index(content[afterStart:], legacyMarker)
	if rel < 0 {
		// Single legacy marker — historically Hero sometimes wrote just one
		// at the top. Treat the whole rest of file as the region so it gets
		// regenerated cleanly into the versioned form.
		body := strings.Trim(content[afterStart:], "\n")
		return ManagedRegion{
			Present:  true,
			Legacy:   true,
...
// RenderManagedRegion produces a managed-region block — start marker, body,
// end marker — at the given version. The body is the caller's
...
// InsertManagedRegion produces a new file content with the managed region
// inserted or updated.
//
// The three cases:
//
//  1. file has no managed region: insert `region` at the top of the file
//     (after the H1 title line if one exists), preserving all existing
//     content below.
...
// IsLegacyHeroStub returns true when content consists entirely of a Hero
// managed region (legacy or versioned) with no user content outside it.
// Used to detect "this CLAUDE.md is a Hero stub from a prior install and
// can safely be replaced with a symlink/shim" — vs the user-authored case
// where any meaningful content exists outside Hero markers.
//
