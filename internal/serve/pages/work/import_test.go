...
// TestNoForbiddenImports asserts that the Work home package does NOT
...
// the hero-work-home spec's boundaries section.
func TestNoForbiddenImports(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("read work dir %s: %v", wd, err)
	}
		...
		for _, imp := range file.Imports {
			...
			for _, bad := range forbidden {
				if val == bad || strings.HasPrefix(val, bad+"/") {
					t.Errorf("%s: forbidden import %q (work must not depend on chat/runner)", entry.Name(), val)
				}
			}
		}
	}
