...
func Check(init *spec.Spec, all []*spec.Spec) CheckResult {
	...
	verdict, next, pause := step(pending, completed, mode)
	res.Verdict = verdict
	if next != nil {
...
func step(pending []*spec.Spec, completed map[string]bool, mode AutonomyMode) (string, *spec.Spec, *PauseInfo) {
	var next *spec.Spec
	blocked := false
...
func DryRun(init *spec.Spec, all []*spec.Spec, n int) []DryStep {
	mode := ParseMode(init.Autonomy)
	kids := Children(init, all)
	completed := completedSet(all)
